    ...
    public Output run(RunContext runContext) throws IllegalVariableEvaluationException, HttpClientException {
        ...
        if (zoneId != null) {
            ...
            if (body == null || !body.success() || body.result() == null) {
                throw new IllegalStateException("Failed to fetch zone by ID: " + rZoneId);
            }

            return buildOutput(body.result());
        }

        String rHostname = runContext.render(hostname).as(String.class).orElseThrow();
        logger.info("Fetching zone by hostname '{}'", rHostname);
        ...
        if (body == null || !body.success() || body.result() == null || body.result().isEmpty()) {
            throw new IllegalStateException("Zone not found for hostname: " + rHostname);
        }
